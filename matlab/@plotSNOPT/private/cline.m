...
function h = cline(self, x, y, z, cdata)
    ...
    if (nargin == 4 && (~isnumeric(z) || ~isvector(z) || length(x)~=length(z))) || ...
       (nargin == 5 && ~isempty(z) && (~isnumeric(z) || ~isvector(z) || length(x)~=length(z))) ...
       (nargin == 5 && (~isnumeric(cdata) || ~isvector(cdata) || length(x)~=length(cdata)))
